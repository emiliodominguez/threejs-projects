...
export default class Ring {
	...
	constructor(options: RingOptions) {
		...
		const extrudeOptions = { depth, bevelEnabled: false, curveSegments: 100 };
		...
		const material = new THREE.MeshNormalMaterial({ side: THREE.DoubleSide, wireframe: options.wireframe ?? false });
